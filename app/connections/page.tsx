...
import { IMAGE_ASSETS, SAMPLE_USERS } from "@/lib/image-assets"
        ...
        <TabsContent value="connected" className="space-y-4 mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {connectedUsers.map((connection) => (
              <div key={connection.id} className="wavv-card rounded-xl p-6 hover:bg-white/5 transition-colors">
                <div className="flex items-start space-x-4 mb-4">
                  <div className="h-12 w-12 rounded-full bg-gradient-to-br from-purple-600 to-indigo-600 flex-shrink-0 overflow-hidden flex items-center justify-center text-xs font-bold">
                    <img src={connection.imageUrl} alt={connection.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-white">{connection.name}</p>
                    <p className="text-xs text-white/70 line-clamp-1">{connection.bio}</p>
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {connection.interests.map((interest) => (
                    <Badge key={interest} variant="secondary" className="bg-purple-600/30 text-white text-xs border border-purple-500/30">
                      {interest}
                    </Badge>
                  ))}
                </div>
